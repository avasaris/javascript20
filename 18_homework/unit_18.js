...
function t3() {
    ...
    a3.forEach(item => item === +item && a3_res.push(item));
...
function t4() {
    const t4 = document.querySelectorAll('span.task-4');
    t4.forEach(item => a4_res.push(item.attributes.data.value));
...
function t10() {
    ...
    Object.entries(a10).forEach(([key, val]) => {
        a10_arr.push(`${key}=${val}`);
    });

    a10_res = a10_arr.join('&');
